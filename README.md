...
There's no model dropdown anymore — each provider always uses its current flagship "latest" model, configured server-side (`GEMINI_DEFAULT_MODEL`, `CLAUDE_DEFAULT_MODEL`, `OPENAI_DEFAULT_MODEL`, `OPENROUTER_DEFAULT_MODEL` in `.env`). The pooled OpenRouter tier defaults to `nvidia/nemotron-3-ultra-550b-a55b:free`. Double-check these against each provider's current docs before deploying — model IDs are a moving target and the shipped defaults may drift out of date.

**OpenRouter's free-tier models rotate without warning** — a model that's free today can 404 with "the free model has been deprecated" tomorrow, no announcement (this happened to an earlier default here within a day of being verified live). Rather than just fail, `llm_call()` (`router.py`) automatically retries once against `OPENROUTER_FALLBACK_MODEL` when the pinned model fails outright — it defaults to `openrouter/free`, OpenRouter's own router across whatever free models are currently live, so a single deprecation doesn't take the whole pooled tier down. The tradeoff is consistency: since the fallback (and `openrouter/free` itself) can pick a different underlying model per request, quality/style can vary bullet-to-bullet if the pinned model ever does fail over. If analysis starts failing outright even with the fallback in place, look up a current model at [openrouter.ai/models](https://openrouter.ai/models) and update `OPENROUTER_DEFAULT_MODEL`.
...
`CRITIC_MODEL` is independent of that same-as-main toggle — it always overrides just the *model*, never the provider/key, so the critic can run a different model from the main agent even while sharing its credentials. It's left blank by default, so the critic just inherits whatever model the main request used — both currently resolve to `nvidia/nemotron-3-ultra-550b-a55b:free` via `OPENROUTER_DEFAULT_MODEL`, and the critic's calls get the same automatic `openrouter/free` fallback on failure described above. Set `CRITIC_MODEL` explicitly only if you want the critic deliberately pinned to something other than whatever the main agent is using.
