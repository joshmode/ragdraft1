...
There's no model dropdown anymore — each provider always uses its current flagship "latest" model, configured server-side (`GEMINI_DEFAULT_MODEL`, `CLAUDE_DEFAULT_MODEL`, `OPENAI_DEFAULT_MODEL`, `OPENROUTER_DEFAULT_MODEL` in `.env`). The pooled OpenRouter tier defaults to `openai/gpt-oss-20b:free`. Double-check these against each provider's current docs before deploying — model IDs are a moving target and the shipped defaults may drift out of date.

**OpenRouter's free-tier models rotate without warning** — a model that's free today can 404 with "the free model has been deprecated" tomorrow, no announcement. If analysis suddenly starts failing, that's the first thing to check: look up the current model at [openrouter.ai/models](https://openrouter.ai/models) and update `OPENROUTER_DEFAULT_MODEL` (or `CRITIC_MODEL`). OpenRouter also offers `openrouter/free`, an auto-router that always picks from whatever free models are currently live — more resilient against this churn, at the cost of inconsistent model behavior request-to-request, since it can route different bullets in the same resume to different underlying models.
